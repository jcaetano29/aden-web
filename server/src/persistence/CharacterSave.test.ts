...
describe("toCharacterSave", () => {
  it("serializa nivel/exp/pos e inventario a Record", () => {
    ...
    const save = toCharacterSave({ level: 3, exp: 40, x: 12, z: -7, inventory: inv, gold: 100, questId: "q1", questProgress: 2 });
    expect(save).toEqual({ level: 3, exp: 40, pos_x: 12, pos_z: -7, inventory: { gold: 5, bone: 2 }, gold: 100, questId: "q1", questProgress: 2 });
  });
});
